...
#' @section Methods:
#' \describe{
#'   \item{transpile()}{Translate the SAS `PROC SQL` statement into R code.}
#' }
#'
#' @import S7
#'
...
extract_table_name <- function(from_section){
  ...
  attributes(table_name)$alias <- table_alias
  return(table_name)
}


mutate_join <- function(from_table, join_table, join_expression){
...
extract_joins <- function(join_sections){
...
sql_to_dplyr <- function(code_sql) {
  ...
  if (any(grepl(x = sentence$key_word, pattern = "join"))) {
    ...
    dplyr_join <-  sapply(1:length(tables_jointures), function(i) {
      ...
      joingned_expression <- mutate_join(
